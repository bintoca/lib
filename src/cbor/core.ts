...
export const createDefaults = () => {
    const defaultTypeMap = new Map<Function, (a, state: EncoderState) => void>([[Object, encodeObject], [NullConstructor, encodeObject], [ArrayBuffer, binaryItem],
    ...
    const defaultTagMap = new Map<number | bigint, (v, state: DecoderState) => any>([[tags.dateString, (v) => new Date(v)], [tags.datePOSIX, (v) => new Date(v * 1000)], [tags.extendedTime, (v) => new Date((v.get(1) || 0) * 1000 + (v.get(-3) || 0))],
    ...
    const defaultNamedConstructorMap = new Map<string, (v, state: DecoderState) => any>([
    ...
    return { defaultTypeMap, defaultTagMap, defaultNamedConstructorMap }
}
export const { defaultTypeMap, defaultTagMap, defaultNamedConstructorMap } = createDefaults()