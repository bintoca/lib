...
export const createParser = (liteProfile?: boolean) => cp(liteProfile) as ParseFullState
export const parseFull = (st: ParseFullState) => {
    try {
        while (true) {
            parseCore(st)
            if (st.decoder.endOfBuffer) {
                st.error = { error: r.incomplete_stream }
                break
            }
        ...
        st.error = { error: r.error_internal }
        st.internalError = e
    }
...
test.each([
...
])('parseError(%#)', (i, o) => {
    {
        ...
        expect(st.error).toEqual({ error: r.incomplete_stream })
    }
    {
        const st = createParser()
