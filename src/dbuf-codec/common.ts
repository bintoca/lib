...
export type Node<T extends ArrayBufferLike = ArrayBufferLike> = { type: number, registry?: number, needed?: number, children?: Node<T>[], val?: number, op?: ParseOp, ops?: ParseOp[], bitSize?: number, arraySize?: number, rootMagic?: boolean, rootLittleEndian?: boolean, choiceShared?: boolean, choiceArray?: boolean, arrayOffset?: number, u8?: Uint8Array<T> }
...
export const concatBuffers = <T extends ArrayBufferLike = ArrayBufferLike>(buffers: Uint8Array<T>[]): Uint8Array<T> => {
    if (buffers.length == 1) {
        return buffers[0]
    }
    const length = buffers.reduce((a, b) => a + b.byteLength, 0)
    const u = new Uint8Array((buffers[0] instanceof SharedArrayBuffer ? new SharedArrayBuffer(length) : new ArrayBuffer(length)) as T)
    let offset = 0
