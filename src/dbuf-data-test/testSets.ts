...
export const testUnpackRefine = (test, expect, unpack: (n: Node, unsafeExpand?: boolean) => any, refineValues: (v) => any) => {
    test.each([
        ...
        [root(type_array(type_choice_array(type_map(r.integer_signed, r.parse_varint), [map(4), map(5), map(6)], type_map(r.value, r.parse_varint))), array(choice(bit_val(2, 2)), choice(bit_val(0, 2), map(7)))), [5, ob(r.value, 7)]],
        [root(type_array(type_choice_array_no_children(r.denominator, 5, type_map(r.value, r.parse_varint))), array(choice(bit_val(7, 3)), choice(bit_val(2, 3)), choice(bit_val(0, 3), map(7)))), ema(1).concat([ss(r.denominator), ob(r.value, 7)])],
        [root(type_array(type_choice_shared_array_no_children(r.denominator, 5, type_map(r.value, r.parse_varint))), array(choice_shared(bit_val(7, 3)), choice_shared(bit_val(2, 3)), choice(bit_val(0, 3), map(7)))), ema(1).concat([ss(r.denominator), ob(r.value, 7)])],
        [root(type_array(type_choice_shared_array(type_map(r.integer_signed, r.parse_varint), [map(4), map(5), map(6)], type_map(r.value, r.parse_varint))), array(choice_shared(bit_val(2, 2)), choice(bit_val(0, 2), map(7)))), [5, ob(r.value, 7)]],
        [root(type_array(type_choice(r.parse_varint, type_map(r.copy_length, r.parse_varint))), array(choice(bit_val(0, 1), 2), choice(bit_val(0, 1), 3), choice(bit_val(1, 1), map(4)))), [2, 3, 3, 3, 3, 3, 3]],
        [root(type_array(type_choice(r.parse_varint, type_map(r.copy_length, r.parse_varint))), array(choice(bit_val(1, 1), map(1)))), ema(2)],
        [root(type_array(type_choice(r.parse_varint, type_map(r.copy_distance, r.parse_varint))), array(choice(bit_val(0, 1), 2), choice(bit_val(0, 1), 3), choice(bit_val(1, 1), map(3)))), [2, 3].concat(ema(1))],
        [root(type_array(type_choice(r.parse_varint, type_map(r.copy_distance, r.parse_varint))), array(choice(bit_val(0, 1), 2), choice(bit_val(0, 1), 3), choice(bit_val(1, 1), map(1)))), [2, 3, 2]],
        [root(type_array(type_choice(r.parse_varint, type_map(r.copy_distance, r.parse_varint))), array(choice(bit_val(1, 1), map(1)))), ema(1)],
        [root(type_array(type_choice(r.parse_varint, type_map(r.copy_distance, r.copy_length, r.parse_varint, r.parse_varint))), array(choice(bit_val(0, 1), 2), choice(bit_val(0, 1), 3), choice(bit_val(1, 1), map(1, 4)))), [2, 3, 2, 3, 2, 3, 2]],
        ...
        [root(type_array(type_choice(r.parse_varint, type_map(r.copy_distance, r.copy_length, type_map(r.offset_add, r.value, parse_type_data_immediate(r.parse_varint, 2), r.parse_varint), type_map(r.offset_add, r.value, parse_type_data_immediate(r.parse_varint, 3), r.parse_varint)))), array(choice(bit_val(0, 1), 2), choice(bit_val(0, 1), 3), choice(bit_val(0, 1), 4), choice(bit_val(0, 1), 5), choice(bit_val(1, 1), map(map(1), map(3))))), [2, 3, 4, 5, 2, 3, 4, 5, 2, 3, 4]],
        ...
        [root(type_map(r.exponent_base2, r.value, parse_bit_size(8), parse_bit_size(6)), map(bit_val(10, 8), bit_val(1, 6))), 8.125],
        ...
        [root(type_map(r.sign, r.exponent_base2, r.value, r.true, parse_bit_size(8), parse_bit_size(6)), map(bit_val(10, 8), bit_val(1, 6))), -8.125],
        [root(type_map(r.exponent_base10, r.value, parse_bit_size(8), parse_bit_size(6)), map(bit_val(3, 8), bit_val(1, 6))), 0.0001],
        ...
        [root(type_map(r.sign, r.exponent_base10, r.value, r.true, type_map(r.integer_signed, parse_bit_size(4)), parse_bit_size(6)), map(map(bit_val(14, 4)), bit_val(1, 6))), -0.000000001],
...
export const testUnpackRefineSafe = (test, expect, unpack: (n: Node, unsafeExpand?: boolean) => any, refineValues: (v) => any) => {
    test.each([
    ...
    ])('unpackRefine_safe(%#)', (i, o) => {
        const a = refineValues(unpack(i, false))
        expect(a).toStrictEqual(o)
    })
}
export const testPack = (test, expect, pack: (v) => Node) => {
    test.each([
...
export const testGetLeapSecondsFromPosix = (test, expect, getLeapSecondsFromPosix: (v) => number) => {
    test.each([['1970-01-01T00:00:00Z', 10], ['1972-12-31T23:59:59Z', 11], ['1973-01-01T00:00:00Z', 12]])('getLeapSecondsFromPosix(%#)', (d, o) => {
        expect(getLeapSecondsFromPosix(Date.parse(d) / 1000)).toBe(o)
    })
}
export const testGetLeapSecondsFromTAI = (test, expect, getLeapSecondsFromTAI: (v) => number) => {
    test.each([['1970-01-01T00:00:00Z', 10], ['1972-12-31T23:59:59Z', (3 * 365 + 1) * 86400 + 11 - 1], ['1973-01-01T00:00:00Z', (3 * 365 + 1) * 86400 + 11], ['1973-01-01T00:00:00Z', (3 * 365 + 1) * 86400 + 12]])('getLeapSecondsFromTAI(%#)', (d, o) => {
        expect(o - getLeapSecondsFromTAI(o)).toBe(Date.parse(d) / 1000)
    })
}